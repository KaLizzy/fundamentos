...
En esta sección discutiremos cuál el objetivo general de el proceso de estimación. 
...
la distribución de $T$ esté **altamente concentrada** alrededor del verdadero
valor $\theta$.
...
Por ejemplo, veamos qué pasa cuando hacemos la muestra más grande en nuestro ejemplo:
...
1. La media muestral $T_1(X) = \frac{1}{n}\sum_{i = 1}{2} X_i$ 
2. El máximo de la muestra $T_2(X) = \max_{i=1,\ldots, 12} X_i$
...
ggplot(sim_estimador_1, aes(x = estimacion)) + geom_histogram() +
  xlim(c(0, 1))
```

```{r}
sim_estimador_2 <- replicar_muestreo_unif(max, 4000, 15)
ggplot(sim_estimador_2, aes(x = estimacion)) + geom_histogram() +
  xlim(c(0, 1))
```
...
Una primera medida útil para medir 
